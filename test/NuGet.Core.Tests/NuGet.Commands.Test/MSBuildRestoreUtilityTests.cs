    ...
    public class MSBuildRestoreUtilityTests
    {
        ...
        public void MSBuildRestoreUtility_GetPackageSpec_NetCore_VerifyDuplicateItemsAreIgnored()
        {
            using (var workingDir = TestDirectory.Create())
            {
                ...
                // Assert
                // Verify no exceptions from creating the spec.
                // Verify dependency counts match.
                Assert.Equal(0, projectSpec.Dependencies.Count);
                Assert.Equal(2, projectSpec.GetTargetFramework(NuGetFramework.Parse("net46")).Dependencies.Count);
                Assert.Equal(2, projectSpec.GetTargetFramework(NuGetFramework.Parse("netstandard1.6")).Dependencies.Count);

                // Verify framework assembly found.
                var systemIO = projectSpec.GetTargetFramework(NuGetFramework.Parse("net46")).Dependencies.Single(e => e.Name == "System.IO");
                Assert.NotNull(systemIO);
            }
        }
